...
class DenseBlock(nn.Module):
    ...
    def forward(self, inputs):
        return inputs + self.norm(self.mlp(inputs))
...
class DenseModel(nn.Module):
    def __init__(self, input_dims):
        ...
        self.input_up_proj = nn.Sequential(
            nn.Linear(input_dims, int(input_dims*2)),
        )

        self.dense_layers = nn.ModuleList([DenseBlock(input_dims*2) 
                                           for _ in range(6)])

        self.output_down_proj = nn.Sequential(
            nn.Linear(int(input_dims*2), input_dims),
    ...
    def forward(self, inputs, labels=None):
        inputs = self.norm(inputs)                      # [batch_size, N]
        x = self.input_up_proj(inputs)                  # [batch_size, 2N]
        for dense in self.dense_layers:
            x = dense(x)                                # [batch_size, 2N]
...
def train(args, device):
    ...
    for epoch in trange(args.epochs):
        ...
        for data in train_dataloader:
            ...
            all_acc.append(accuracy_score(labels.cpu().numpy(), 
                                          prediction))
            scheduler.step()
        ...
        with torch.no_grad():
            ...
            for dev_inputs, dev_labels in dev_dataset:
                ...
                all_dec_acc.append(accuracy_score(dev_labels.cpu().numpy(),
                                                   dev_prediction))
...
def inference(model, device):
    dataloader = create_data_loader('test', None)
    model.eval()
    results = {'label': []}
    for data in dataloader:
        with torch.no_grad():
            ...
            prediction = (prediction_scores > 0.5).int().cpu().numpy()
            results['label'].extend(prediction.reshape(-1))
